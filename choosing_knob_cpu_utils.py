...
if __name__ == '__main__':
    ...
    for log_dir in dirs:
        ...
        if thread_num == 1:
            thread_num += 1
            axes[row, col].set_title(key_seq[row] + " " + str(thread_num) + " thds")
        else:
            axes[row, col].set_title(key_seq[row] + " " + str(thread_num) + " thds")
        ...
        line_label = str(batch_size_list[color])
        if thread_num == 1:
            thread_num += 1
        line = axes[row, col].plot(stat_df["cpu_percent"] / thread_num, color=color_list[color], label=line_label,
    ...
    print(lines)
    leg = fig.legend(lines[1:], ["128 MB", "64 MB"],
                 loc='lower center', frameon=True)

    # axes[0, 0].add_artist(leg)
    axes[3, 1].set_xlabel("time elapsed (sec)")
    axes[3,1].set_xlabel("time elapsed (sec)")
    axes[3, 1].set_xlabel("time elapsed (sec)")
    ...
    plt.savefig("paper_usage/knob_choosing/cpu_utils_in_one_page_all.png")
    plt.savefig("paper_usage/knob_choosing/cpu_utils_in_one_page_all.pdf")
